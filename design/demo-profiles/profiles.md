...
사진이 들어오면 아래 블록을 [`app/src/lib/demo-profiles.ts`](../../app/src/lib/demo-profiles.ts)의
`DEMO_PROFILES`에 그대로 붙인다. 사진 파일이 없으면 `require()`가 번들을 깨뜨리니, **사진이
들어온 사람만** 옮긴다.
...
밤의라디오·주말텃밭은 2026-09-09에 사진이 들어와 `demo-profiles.ts`로 옮겼다.
아래 블록은 기록으로만 둔다.
