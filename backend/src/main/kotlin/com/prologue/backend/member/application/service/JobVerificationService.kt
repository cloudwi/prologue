 ...
 * 성공하면 **도메인만** 남긴다(이메일 전체는 저장하지 않는다 — 필요한 건 "어디 소속인가"뿐).
...
class JobVerificationService(
    ...
    fun verify(accountId: UUID, rawEmail: String, rawCode: String): String {
        ...
        jdbc.update(
            """
            insert into job_verifications (account_id, email_domain, verified_at) values (?, ?, now())
            on conflict (account_id) do update set email_domain = excluded.email_domain, verified_at = now()
            """.trimIndent(),
            accountId, domain,
        )
        return domain
