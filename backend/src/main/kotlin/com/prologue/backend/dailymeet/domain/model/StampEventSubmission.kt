...
/**
 * 우표 이벤트 제출 — 블로그 후기 링크 하나. 운영자가 검토해 승인(우표 지급) 또는 반려한다.
 * 한 번 처리된 제출은 되돌릴 수 없다 — 지급이 원장에 이미 남았기 때문.
 */
class StampEventSubmission private constructor(
    ...
    companion object {
        ...
        fun submit(accountId: UUID, url: String, now: Instant = Instant.now()): StampEventSubmission {
            ...
            return StampEventSubmission(null, accountId, trimmed, Status.PENDING, null, now, null)
        }
        ...
        fun reconstitute(
            ...
            createdAt: Instant,
            decidedAt: Instant?,
        ): StampEventSubmission = StampEventSubmission(id, accountId, url, status, grantedAmount, createdAt, decidedAt)
    }
}
