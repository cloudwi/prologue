 ...
 * "언제 이어졌는가"를 모아 주고 잉크로 창을 다시 여는 일을 맡는다.
 */
@Service
class ProfileAccessService(
    ...
    /** 이 사용자가 잉크로 열어둔 상대들. 목록 화면은 이걸 한 번 읽어 전부 판정한다. */
     ...
     * 소개만 받고 하트는 오가지 않은 상대도 있다. 편지는 잉크를 쓰고 연락처를 건넨 상대라
    ...
    /**
     * 잉크를 써서 상대 프로필을 다시 연다. 한 번 열면 다시 닫히지 않는다.
     *
     * 이미 열려 있으면 잉크를 쓰지 않고 조용히 성공한다(멱등) — 앱이 재시도하거나
     * 버튼이 두 번 눌렸다고 두 장이 나가면 안 된다. 실패로 답하면 앱이 계속 재시도한다.
     *
     * 아직 창이 열려 있는 상대에게도 잉크를 쓰지 않는다. 지금 공짜로 볼 수 있는 걸
    ...
    fun unlock(accountId: UUID, peerAnswerId: UUID): UnlockResult {
        ...
        if (alreadyUnlocked) return UnlockResult(spent = false, balance = inkService.balance(accountId))
