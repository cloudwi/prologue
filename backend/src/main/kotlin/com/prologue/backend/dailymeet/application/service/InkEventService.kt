...
/**
 * 잉크 이벤트 — 블로그 후기 링크를 제출하면 운영자가 검토 후 잉크를 지급한다.
 * 제출은 검토 중 1건만(중복 방지), 승인 시 지급은 원장(reason=EVENT)에 남는다.
 */
@Service
class InkEventService(
    private val submissionRepository: InkEventSubmissionRepository,
    private val inkService: InkService,
    private val memberQueryService: MemberQueryService,
) {
    @Transactional
    fun submit(accountId: UUID, url: String): List<InkEventSubmission> {
        if (submissionRepository.existsPendingByAccountId(accountId)) {
            throw DailyMeetException("검토 중인 제출이 있어요. 결과를 기다려주세요.")
        }
        submissionRepository.save(InkEventSubmission.submit(accountId, url))
        return submissionRepository.findByAccountId(accountId)
    }

    @Transactional(readOnly = true)
    fun mySubmissions(accountId: UUID): List<InkEventSubmission> =
    ...
    /** 어드민 — 승인. 잉크 지급과 상태 전이가 한 트랜잭션이라 지급만 되고 기록이 안 남는 일은 없다. */
    @Transactional
    fun approve(submissionId: UUID, amount: Int) {
        ...
        inkService.grantTo(submission.accountId, amount, InkService.REASON_EVENT)
    }
