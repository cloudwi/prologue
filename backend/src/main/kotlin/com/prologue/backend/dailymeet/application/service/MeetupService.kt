 ...
 *
 * `[사진1]`이 기본이고, `[사진1:50]`처럼 폭(%)이, 그 뒤에 `[사진1:100:1200x1115]`처럼
 * 원본 크기가 더 붙을 수 있다. 둘 다 **선택 항목**이라 폭도 크기도 없던 시절의 글이 그대로
 * 읽힌다 — 마이그레이션 없이 문법만 자랐다.
 *
 * 문법이 자랄 때 여기를 같이 고치지 않으면, 새 모양의 표시가 앱 화면에 글자로 샌다.
 */
