...
class AuthExceptionHandler {
    ...
    // [진단용 임시] 이메일 발송 실패 원인(제공자 응답)을 응답에 노출. 안정화 후 제거 예정.
    @ExceptionHandler(EmailSendException::class)
    fun handleEmailSend(e: EmailSendException): ResponseEntity<ErrorResponse> {
        log.error("이메일 발송 실패", e)
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
            .body(ErrorResponse("MAIL_SEND_FAILED", e.message))
    }
